...
Outputs (same as run_diabetes.py):
    ...
    - checkpoints/best.pt (alias of best-by-MSE)
    - checkpoints/best_mse.pt
    - checkpoints/best_xi.pt
"""
...
def load_ecoli_conc_dataset(
    ...
    # ---- STRICT A2 VALIDATION: both ERY and TMP must be present ----
    present_levels = set(df["drug"].dropna().unique().tolist())
    if not {"ERY", "TMP"}.issubset(present_levels):
        raise ValueError(
            f"A2 validity check failed: both ERY and TMP must be present after normalization. "
            f"Observed levels: {sorted(present_levels)}"
        )
...
def main(args: argparse.Namespace) -> None:
    ...
    # overwrite (not append) for cache correctness
