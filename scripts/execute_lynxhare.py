# execute_lynxhare.py
"""
End-to-end executor for Lynx–Hare (Task B2) experiments using the optional Xi regularizer.

Task B2: Predict Lynx_{t+1} from [Hare_t, Lynx_t, Hare_{t-1}, Lynx_{t-1}].

Changes vs legacy:
- Repeated **blocked/rolling splits** (time-series safe) keyed by `seed`
  so resampled test sets overlap -> Nadeau–Bengio corrected t-test applies.
- Dual checkpointing in the runner (best-by-val MSE and best-by-val hard-ξ).
- Aligned selection/evaluation:
    RQ2: choose λ,τ and checkpoint by val MSE -> report test MSE/R².
    RQ3: choose λ,τ and checkpoint by val hard-ξ -> report test hard-ξ.

Outputs are written to
    runs/YYYYMMDD_HHMMSS_lynxhare_exec/
and include:
    ...
    figures/heatmap_xi_xiSel.png
    figures/heatmap_test_mse_mseSel.png
    figures/heatmap_test_r2_mseSel.png
...
# --------------------------------------------------------------------------- #
#  Local imports (assumes run_lynxhare.py sits next to this script)           #
# --------------------------------------------------------------------------- #
sys.path.insert(0, str(Path(__file__).parent))
from run_lynxhare import main as run_lynxhare_main  # type: ignore
import run_lynxhare as rl  # noqa: F401  (kept for completeness)
...
# Synthetic correlation sanity study (unchanged, general-purpose)
SYN_FUNCS = {
    "linear": lambda x: x,
...
TRAIN_RATIO = 0.80
TEST_RATIO = 0.10
NB_C = (1.0 / R) + (TEST_RATIO / TRAIN_RATIO)

# --------------------------------------------------------------------------- #
#  Session directories & logging                                              #
# --------------------------------------------------------------------------- #
STAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
SESSION_DIR = RUNS_ROOT / f"{STAMP}_lynxhare_exec"
...
log.info("===== Lynx–Hare (B2) execution started =====")
...
#  Helpers                                                                    #
# --------------------------------------------------------------------------- #
def run_one_lynxhare(seed: int, use_xi: bool, lam: float, tau: float) -> Dict[str, float]:
    """
    Train one model (or reuse cached results) and return its metrics row.
    Uses repeated blocked/rolling split keyed by `seed` inside run_lynxhare.py.
    """
    ...
    # Build argparse-like namespace expected by run_lynxhare.main()
    args = SimpleNamespace(
        outdir=str(outdir),
        seed=seed,                 # also determines the rolling split window
        use_xi=use_xi,
        lambda_coef=lam,
        tau=tau,
        epochs=EPOCHS,
        ...
        grad_clip=1.0,
        cpu=True,
        data_csv=None,             # use runner's default source
    )

    run_lynxhare_main(args)
    ...
    # Quick sanity threshold on R² for the mse-selected model (time-series task is modest)
    if row["test_r2_mseSel"] < 0.10:
        log.warning(
...
for seed in SEEDS:
    ...
    rows.append(run_one_lynxhare(seed, False, 0.0, 0.0))
    total_runs_done += 1
    ...
    for lam in LAMBDA_SET:
        for tau in TAU_SET:
            rows.append(run_one_lynxhare(seed, True, lam, tau))
            total_runs_done += 1
            remaining = total_runs_expected - total_runs_done
            log.info(
                "run %d completed, %d remaining. est minutes left: %.1f",
...
log.info("Lynx–Hare runs completed - total %d models", total_runs_done)
...
plt.xlabel("tau")
plt.ylabel("lambda")
plt.title("Lynx_{t+1}: Xi dependency by lambda and tau (xi-selected)")
plt.tight_layout()
