...
and include:
    ...
    timing_bench.csv     - Xi overhead numbers (100-step micro-benchmark)
    stats_summary.txt    - paired t-test result (best Xi vs baseline per seed)
...
def run_one_lynxhare(seed: int, use_xi: bool, lam: float, tau: float) -> Dict[str, float]:
    ...
    args = SimpleNamespace(
        ...
        grad_clip=1.0,
        cpu=True,
        data_csv=None,  # use run_lynxhare's default source
    )
    ...
    # Quick sanity threshold: R² should not be absurdly bad on this task
    if row["test_r2"] < 0.10:
        log.warning(
...
#  5. Figure generation                                                       #
# --------------------------------------------------------------------------- #
# Heat map mean xi by lambda and tau (test set)
pivot = (
...
plt.colorbar(label="mean hard xi (test)")
plt.xlabel("tau")
plt.ylabel("lambda")
plt.title("Lynx_{t+1}: Xi dependency by lambda and tau")
plt.tight_layout()
plt.savefig(FIG_DIR / "heatmap_xi.png", dpi=120)
plt.close()

# === EXTRA HEATMAPS: MSE and R2
# Keep baseline (use_xi == 0) so the λ=0, τ=0 cell shows the reference.
metrics = {
    "test_mse" : dict(title="Lynx_{t+1} — Mean Test MSE (lower is better)",
                      cmap ="viridis_r"),   # reversed so low = bright
    "test_r2"  : dict(title="Lynx_{t+1} — Mean Test R2 (higher is better)",
                      cmap ="viridis"),
}
...
# Synthetic scatter plot (Xi vs Spearman vs Pearson)
